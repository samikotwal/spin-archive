...
const FeaturedSlideshow = ({ anime, isLoading }: FeaturedSlideshowProps) => {
  ...
  // Weekly rotation: use week number to offset the slice
  const featured = useMemo(() => {
    if (anime.length === 0) return [];
    const weekNum = Math.floor(Date.now() / (7 * 24 * 60 * 60 * 1000));
    const offset = (weekNum * 3) % Math.max(anime.length - 6, 1);
    return anime.slice(offset, offset + 6).length >= 3
      ? anime.slice(offset, offset + 6)
      : anime.slice(0, 6);
  }, [anime]);
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10 z-10">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <h2 className="text-2xl md:text-4xl font-black text-foreground mb-2 max-w-lg">{item.title}</h2>
            ...
            {item.genres && (
              <div className="flex gap-2 mb-4 flex-wrap">
                {item.genres.slice(0, 3).map((g) => (
                  <Badge key={g.name} variant="outline" className="border-white/20 text-xs">{g.name}</Badge>
                ))}
              </div>
            )}
            <Button
